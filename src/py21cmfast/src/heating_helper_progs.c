...
typedef struct
{
    double ah, gammah, etah, Jx_red_temp_var_x_jsc; // Added Jx_red_temp_var_x_jsc to include more params
} h_params;
*/

//(eq:14) of Reis, Fialkov, Barkana (2021) [2101.01777]

/* This doesn't work on mac, clang forbids nested definitions
double Jx_red(double x, void *params)
{
    double Jx_red_integrand(double z, void *params)
    {
        double jx, xz;
        xz = z * z * z - x * x * x;
...
// clang compatible versions of Jx_red (and Jx_red_integrand)
/* dont need this
double Jx_red_integrand_clang(double z, void *params)
{
    double jx, xz, x;
    h_params *p = (h_params *)params;
    x = p->Jx_red_temp_var_x_jsc;
    xz = z * z * z - x * x * x;
    jx = exp(2. * PI * p->gammah * xz / 3. / p->ah + 2. * p->etah * (z - x)) * z * z;
    return jx * (2. * PI * p->gammah / p->ah);
}
...
double Jx_red(double x, void *params)
{
    ...
    F.function = &Jx_red_integrand_clang;
    F.params = &p;
...
// Moved outside, clang forbids nested definitions
...
double hyrec_dx_H_dz(HYREC_DATA *rec_data, double x_H, double xe, double nH, double z, double Hz, double Tmat, double Trad)
{
    ...
    if (Trad_phys <= TR_MIN || Tmat / Trad <= T_RATIO_MIN)
    {
        model = PEEBLES;
    }
    else
                          ...
                          double H, double Gamma_C, double T_gamma, float dzp, double dT_b_2_dt_ext, double dT_chi_2_dt_ext)
{
    ...
    if (user_params_hf->SDM_TARGET_TYPE == 1)
    { // baryons
        t_max = 1;
        m_t_array[0] = m_b; // g
        n_t_array[0] = n_b; // cm^-3
    }
    else if (user_params_hf->SDM_TARGET_TYPE == 2)
    ...
    else if (user_params_hf->SDM_TARGET_TYPE == 3)
    { // hydrogen nuclei
        t_max = 1;
        m_t_array[0] = _m_H_;                 // g
        n_t_array[0] = No * pow(1. + zp, 3.); // cm^-3
    }
    else if (user_params_hf->SDM_TARGET_TYPE == 4)
    { // protons
        t_max = 1;
        m_t_array[0] = m_p;                                   // g
        n_t_array[0] = x_e * (No + He_No) * pow(1. + zp, 3.); // cm^-3
    }
    else if (user_params_hf->SDM_TARGET_TYPE == 5)
    { // electrons
        t_max = 1;
        m_t_array[0] = m_e;                                   // g
        n_t_array[0] = x_e * (No + He_No) * pow(1. + zp, 3.); // cm^-3
    ...
    if ((fabs(SDM_rates.epsilon_b) <= EPSILON_THRES) || (fabs(SDM_rates.epsilon_chi) <= EPSILON_THRES))
    {
        ...
        dT_bar_chi_b_2_dt = (-2. * H * SDM_rates.T_bar_chi_b + n_b / (n_b + n_chi) * (dT_b_2_dt_Comp + dT_b_2_dt_ext) + n_chi / (n_b + n_chi) * dT_chi_2_dt_ext + 1. / (n_b + n_chi) * (2. / (3. * k_B)) * (rho_b * rho_chi / rho_m) * V_chi_b * SDM_rates.D_V_chi_b); // K/sec
        ...
        for (t = 0; t < t_max; t++)
        {
            d_Gamma_chi_b_2_dz += ((rho_t_array[t] * pow(u_chi_t_array[t], n_SDM + 1.)) / pow(m_chi + m_t_array[t], 2.)                                                                                                                                                                                                                                                                                                                                                                                                                                 // g^-1 * cm^(n_SDM-2) / sec^(n_SDM+1)
                                   * (3. * (beta_n_array[t] - pow(r_t_array[t], 2.) / 3. * alpha_n_array[t]) / (1. + zp) + (r_t_array[t] / u_chi_t_array[t]) * ((n_SDM + 1.) / 3.) * (alpha_n_array[t] - pow(r_t_array[t], 2.) / 5. * gamma_n_array[t]) * dV_chi_b_2_dz - (n_SDM + 1.) * (2. * pow(r_t_array[t], 2.) / 3. * alpha_n_array[t] - beta_n_array[t] - pow(r_t_array[t], 4.) / 15. * gamma_n_array[t]) * (m_t_array[t] + m_chi) / (2. * m_t_array[t] * m_chi) * (k_B / pow(u_chi_t_array[t], 2.)) * dT_bar_chi_b_2_dt * (-1. / H / (1 + zp))) // dimensionless
...
double S_alpha_correction(float xi, int flag)
{
    ...
    if (xi > xi_t[RECFAST_NPTS - 1])
    { // approximate S_alpha = 1 at very large values of xi
        ans = 1.;
    }
    else if (xi < xi_t[0])
    { // approximate S_alpha = 2/9 * xi at very small values of xi
        ans = 2. / 9. * xi;
    }
    else
    { // Do spline
