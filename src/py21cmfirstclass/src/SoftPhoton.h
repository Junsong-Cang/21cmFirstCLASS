/*
Soft photon heating module, some functions are already defined in RadioExcess.h
To test with python do: 
    gcc -shared -o SoftPhoton.so SoftPhoton.c
TODO: 
    All modules must be tested and compared with Chluba's notebook if possible
    Re-write everything so that xe follows 21cmFAST definition
    If we use p21c time-step will we have convergence issue?
    If we use Tk from previous time-step (we don't really have access to current Tk) will it be ok? If using current Tk we might not be able to do Ts easily
NOTE:
    We use 21cmFAST definition of ionization fraction
*/
