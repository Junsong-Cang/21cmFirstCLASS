...
#pragma omp for
        ...
        else
        {
            ...
            if (this_spin_temp->first_box)
            {
                ...
                { // will use the results obtained from recfast
                                    ...
                                    *((float *)delta_baryons + HII_R_FFT_INDEX(i, j, k)) = perturbed_field->baryons_density[HII_R_INDEX(i, j, k)];
                                    ...
                                    if (user_params->SCATTERING_DM)
                                    {
                                        *((float *)delta_SDM + HII_R_FFT_INDEX(i, j, k)) = perturbed_field->SDM_density[HII_R_INDEX(i, j, k)];
                                        ...
                                        if (*((float *)delta_SDM + HII_R_FFT_INDEX(i, j, k)) <= -1)
                                        { // correct for aliasing in the filtering step
                                    ...
                                    for (k = 0; k < user_params->HII_DIM; k++)
                                    {
                                        ...
                                        else
                                        {
                                            if (user_params->USE_RELATIVE_VELOCITIES)
                                            {
                                                ...
                                                if (user_params->SCATTERING_DM)
                                                {
                                                    curr_vcb = ((1. - pow(10., -cosmo_params->f_chi)) * (ini_boxes->lowres_vcb[HII_R_INDEX(i, j, k)]) + pow(10., -cosmo_params->f_chi) * (previous_spin_temp->V_chi_b_box[HII_R_INDEX(i, j, k)]) * (1. + global_params.Z_REC) / (1. + zp));
                                                }
                                                else
                                                {
                            ...
                            if (!NO_LIGHT)
                            {
                                ...
                                for (R_ct = global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct--;)
                                {
                                    ...
                                    if (user_params->USE_INTERPOLATION_TABLES)
                                    {
                                        ...
                                        if (user_params->EVOLVE_BARYONS)
                                        {
                                            dfcoll_dz_val = ST_over_PS[R_ct] * (1. + delNL0_rev_baryons[box_ct][R_ct] * zpp_growth[R_ct]) * (dfcoll_interp1[dens_grid_int_vals[box_ct][R_ct]][R_ct] * (density_gridpoints[dens_grid_int_vals[box_ct][R_ct] + 1][R_ct] - delNL0_rev[box_ct][R_ct]) + dfcoll_interp2[dens_grid_int_vals[box_ct][R_ct]][R_ct] * (delNL0_rev[box_ct][R_ct] - density_gridpoints[dens_grid_int_vals[box_ct][R_ct]][R_ct]));
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        if (user_params->EVOLVE_BARYONS)
                                        {
                                            dfcoll_dz_val = ST_over_PS[R_ct] * (1. + delNL0_rev_baryons[box_ct][R_ct] * zpp_growth[R_ct]) * (dfcoll_dz(zpp_for_evolve_list[R_ct], sigma_Tmin[R_ct], delNL0_rev[box_ct][R_ct], sigma_atR[R_ct]));
                                        }
                                        else
                                        {
