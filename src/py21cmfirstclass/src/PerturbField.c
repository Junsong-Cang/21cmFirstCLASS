                ...
                for (k = 0; k < dimension; k++)
                {
                    if (HIRES_FLAG)
                    {
                        *((float *)box + R_INDEX(i, j, k)) = *((float *)FFT_dummy_box + R_FFT_INDEX(i, j, k));
                        if (DENSITY_FLAG && *((float *)box + R_INDEX(i, j, k)) <= -1)
                    ...
                    else
                    {
                        *((float *)box + HII_R_INDEX(i, j, k)) = *((float *)FFT_dummy_box + HII_R_FFT_INDEX(i, j, k));
                        if (DENSITY_FLAG && *((float *)box + HII_R_INDEX(i, j, k)) <= -1)
                        { // correct for aliasing in the filtering step
    ...
    struct InitialConditions *boxes, struct PerturbedField *perturbed_field)
{
    ...
    */
    ...
    int status;
    Try
    { // This Try{} wraps the whole function, so we don't indent.
        ...
        if (!user_params->USE_DICKE_GROWTH_FACTOR || user_params->EVOLVE_BARYONS || user_params->EVOLVE_MATTER)
        {
            ...
            if (redshift < 5.5)
            {
                printf("init_CLASS_GROWTH_FACTOR 1\n");
            }
            */
            ...
            if (redshift < 5.5)
            {
                printf("init_CLASS_GROWTH_FACTOR 2\n");
            }
            */
        }
                        ...
                        for (k = 0; k < user_params->DIM; k++)
                        {
                            ...
                            if (user_params->CLOUD_IN_CELL)
                            {
                                ...
                                if (xf < (double)(xi + 0.5))
                                {
                                    ...
                                    {
                                        xi += (dimension);
                                    } // Only this critera is possible as iterate back by one (we cannot exceed DIM)
                                }
                                if (yf < (double)(yi + 0.5))
                                {
                                    d_y = 1. - d_y;
                ...
                for (j = 0; j < user_params->HII_DIM; j++)
                {
                    for (k = 0; k < user_params->HII_DIM; k++)
                    {
                        *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) /= (float)HII_TOT_NUM_PIXELS;
                        ...
                        if (*((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) < -1.0)
                        { // shouldn't happen

                            if (bad_count < 5)
                                LOG_WARNING("LOWRES_density_perturb is <-1 for index %d %d %d (value=%f)", i, j, k, *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)));
                            if (bad_count == 5)
                                LOG_WARNING("Skipping further warnings for LOWRES_density_perturb.");
                            *((float *)LOWRES_density_perturb + HII_R_FFT_INDEX(i, j, k)) = -1 + FRACT_FLOAT_ERR;
                            bad_count++;
                        }
                    }
...
#pragma omp for
        ...
        if (!user_params->USE_DICKE_GROWTH_FACTOR || user_params->EVOLVE_BARYONS || user_params->EVOLVE_MATTER)
        {
            ...
            if (redshift < 5.5)
            {
                printf("destruct_CLASS_GROWTH_FACTOR 1\n");
            }
            */
            ...
            if (redshift < 5.5)
            {
                printf("destruct_CLASS_GROWTH_FACTOR 2\n");
            }
            */
        }
